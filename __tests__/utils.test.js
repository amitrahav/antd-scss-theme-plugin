...
import { compileThemeVariables, extractLessVariables, loadScssThemeAsLess } from '../src/utils';
...
describe('extractLessVariables', () => {
	it('should correctly extract computed variables', async () => {
		const extractedVariables = await extractLessVariables(path.join(__dirname, 'data', 'test.less'));
	...
	it('should properly overload variables', async () => {
		const extractedVariables = await extractLessVariables(path.resolve(__dirname, 'data', 'test.less'), {
